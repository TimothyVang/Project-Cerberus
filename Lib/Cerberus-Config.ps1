...
function Get-CerberusConfig {
    ...
    if (-not $Config.MinIO) {
        ...
        if (-not $Config.MinIO.Server -or $Config.MinIO.Server -eq "YOUR_MINIO_SERVER:PORT") {
            $errors += "MinIO.Server not configured"
        }
        if (-not $Config.MinIO.AccessKey -or $Config.MinIO.AccessKey -eq "YOUR_ACCESS_KEY") {
            $errors += "MinIO.AccessKey not configured"
        }
        if (-not $Config.MinIO.SecretKey -or $Config.MinIO.SecretKey -eq "YOUR_SECRET_KEY") {
