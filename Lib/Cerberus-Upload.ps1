...
function Get-RemoteFileInfo {
    ...
    # WHY set MC_HOST_minio?
    # MinIO's CLI (mc.exe) reads credentials from environment variables named
    # MC_HOST_<alias>. The "minio" alias lets us reference paths as "minio/bucket/file".
    # Format: https://ACCESS_KEY:SECRET_KEY@SERVER:PORT
    $env:MC_HOST_minio = "https://$($Config.MinIO.AccessKey):$($Config.MinIO.SecretKey)@$($Config.MinIO.Server)"

    $bucket = $Config.MinIO.Bucket
    # WHY "minio/" prefix?
    # "minio" is the alias we set in MC_HOST_minio above. mc.exe uses it to
    # look up the credentials and server address for this path.
...
function Send-ToMinIO {
    ...
    # Set MC_HOST_minio env var so mc.exe knows our server credentials.
    # See Get-RemoteFileInfo above for a detailed explanation of this pattern.
    $env:MC_HOST_minio = "https://$($Config.MinIO.AccessKey):$($Config.MinIO.SecretKey)@$($Config.MinIO.Server)"
